...
void ArenaTeam::SetCaptain(const uint64& guid)
{
    ...
    if(newcaptain)
    {
        ...
        TC_LOG_DEBUG("arena", "Player: %s [GUID: " UI64FMTD "] promoted player : %s[GUID:" UI64FMTD "] to leader of arena team[Id:%u][Type:%u].", oldcaptain->GetName().c_str(), oldcaptain->GetGUID(), newcaptain->GetName().c_str(), newcaptain->GetGUID(), GetId(), GetType());
        LogsDatabase.PExecute("INSERT INTO arena_team_event (id, event, type, player, ip, time) VALUES (%u, %u, %u, %u, '%s', %u)",
