    ...
    else
    {
        ...
        switch (type)
        {
            ...
            case SMART_EVENT_FRIENDLY_KILLED:
            {
                ...
                {
                    TC_LOG_ERROR("sql.sql","SmartAIMgr: Entry %d SourceType %u Event %u Action %u has range (%u) above max possible value, range has been capped to %u.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), e.event.friendlyDeath.range, CREATURE_MAX_DEATH_WARN_RANGE);
