...
void EscortMovementGenerator<T>::DoInitialize(T* unit)
{
    ...
    //Call for creature group update for first move. Next are handled in SplineHandler class
    if(Creature* creature = unit->ToCreature())
        if (creature->GetFormation() && creature->GetFormation()->getLeader() == creature)
            creature->GetFormation()->LeaderMoveTo(m_precomputedPath[1].x, m_precomputedPath[1].y, m_precomputedPath[1].z, !unit->IsWalking());
