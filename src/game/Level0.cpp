...
bool ChatHandler::HandleBuyInShopCommand(const char *args)
{
    ...
    if (!query) 
    {
        PSendSysMessage("Commande inconnue ou cr�dits insuffisants. V�rifiez que vous avez correctement entr� la commande.");
        SetSentErrorMessage(true);
        return false;
    }
...
bool ChatHandler::HandleRaceOrFactionChange(const char* args)
{
    ...
    if (m_session->GetPlayer()->getLevel() < 10) {
        PSendSysMessage("Vous devez �tre au minimum niveau 10 pour �tre �ligible � un changement de faction.");
        SetSentErrorMessage(true);
        return false;
    }
