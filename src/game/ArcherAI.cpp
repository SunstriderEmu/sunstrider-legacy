...
ArcherAI::ArcherAI(Creature *c) : CreatureAI(c)
{
    ...
    if (!sSpellStore.LookupEntry(me->m_spells[0])) {
    ...
    m_minRange = GetSpellMinRange(sSpellRangeStore.LookupEntry(sSpellStore.LookupEntry(me->m_spells[0])->rangeIndex));
    if (!m_minRange)
        m_minRange = MELEE_RANGE;
    m_CombatDistance = GetSpellMaxRange(sSpellRangeStore.LookupEntry(sSpellStore.LookupEntry(me->m_spells[0])->rangeIndex));
