...
class ChatHandler
{
    public:
        ...
        virtual void SendSysMessage(          int32     entry);
        virtual void PSendSysMessage(         const char *format, ...) ATTR_PRINTF(2,3);
        virtual void PSendSysMessage(         int32     entry, ...  );
	...
	bool HandleAccountMailChangeCommand(const char* args);
        ...
        bool HandleRecupParseCommand(Player *player, std::string command, uint32 metier_level, bool equip = false);
