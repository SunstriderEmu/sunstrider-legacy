...
unsigned int itemguid[LIST_SIZE]  = {
197,70,134,76,78,144
};

DELETE FROM character_inventory WHERE 
item IN ( 197,70,134,76,78,144 );
DELETE FROM mail_items WHERE
item_guid IN ( 197,70,134,76,78,144 );
*/

bool ChatHandler::GetItemDataByGuid(Tokens& tokens, unsigned int guid)
{
    QueryResult* result = CharacterDatabase.PQuery("SELECT data FROM item_instance WHERE guid=%u", guid);
    ...
    if (!fields) 
    {
        PSendSysMessage("ChatHandler::GetItemDataByGuid : Empty set.");
        return false;
    }

    tokens = StrSplit(fields[0].GetString(), " ");

    if(tokens.size() < 15)
    {
        PSendSysMessage("ChatHandler::GetItemDataByGuid : Lolwtf. Incorrect data size.");
        PSendSysMessage("Data : %s",fields[0].GetString());
        return false;
    }
...
ItemPrototype const* ChatHandler::GetItemProtoInTokens(Tokens& tokens)
{
    unsigned item_entry = atoi(tokens[3].c_str());

    return objmgr.GetItemPrototype(item_entry);
}

unsigned int ChatHandler::GetItemCountInTokens(Tokens& tokens)
{
    return atoi(tokens[14].c_str());
}

bool ChatHandler::ItemIsInMailOrInventory(unsigned int guid)
{
    QueryResult* result = CharacterDatabase.PQuery("SELECT null FROM mail_items WHERE item_guid=%u",guid);
    if(result)
        return true;
    
    delete result;
    result = CharacterDatabase.PQuery("SELECT null FROM character_inventory WHERE item=%u",guid);
    if(result)
        return true;
...
bool ChatHandler::HandleListOrphanedFor(const char* args) {
    ...
    unsigned int playerguid = atoi(args);
    if (!playerguid)
        return false;
    ...
    if (!found)
    {
        PSendSysMessage("Personnage %u introuvable ou aucun objet dans la liste.",playerguid);
        return true;
    }

    PSendSysMessage("Liste des objets 'orphelins' pour le joueur %u :",playerguid);

    for (int i = startindex; i < LIST_SIZE && ownerguid[i] == playerguid; i++)
    {
        Tokens tokens;
        if (!GetItemDataByGuid(tokens,itemguid[i]))
        {
            PSendSysMessage("Objet trouvable (guid : %u)", itemguid[i]);
            continue;
        }

        if (ItemIsInMailOrInventory(itemguid[i]))
        {
            PSendSysMessage("HandleListOrphanedFor : Item %u is already in mail or inventory",itemguid[i]);
            continue;
        }

        ItemPrototype const* proto = GetItemProtoInTokens(tokens);
        if (proto)
            PSendSysMessage("guid : %u - id : %u (%s) x %u",itemguid[i], proto->ItemId, proto->Name1, GetItemCountInTokens(tokens));
        else
            PSendSysMessage("HandleListOrphanedFor :: cannot get item proto (guid : %u)", itemguid[i]);
    }
...
bool ChatHandler::HandleListMostOrphaned(const char* args) {
    unsigned int minEntries = atoi(args);
    if (!minEntries)
        return false;
    ...
    std::vector<unsigned int> truecount;
    ...
    for(int i = 0; i < LIST_SIZE; i++)
    {
        ...
        if (ItemIsInMailOrInventory(itemguid[i]))
        {
            // PSendSysMessage("HandleListMostOrphaned : Item %u is already in mail or inventory",itemguid[i]); //too much spam
            continue;
        }
        ...
        if (proto)
        {
            unsigned entry = proto->ItemId;
            if(unsigned int index = std::find(entries.begin(), entries.end(), entry) == entries.end())
            {
                entries.push_back(entry);
                std::string* name = new std::string(proto->Name1);
                names.push_back(name);
                count.push_back(1);
                truecount.push_back(GetItemCountInTokens(tokens));
            } else {
                entries[index] = entry;
                count[index] = count[index] + 1;
                truecount[index] = truecount[index] + GetItemCountInTokens(tokens);
            }
        } else {
            PSendSysMessage("HandleListOrphanedFor :: cannot get item proto (guid : %u)", itemguid[i]);
        }
    ...
    PSendSysMessage("Liste des objets représentés au moins %ux :",minEntries);
    for (int i = 0; i < entries.size(); i++)
    {
       // PSendSysMessage("%s",*(names[i]));
        if(count[i] >= minEntries)
            PSendSysMessage("Entry : %u, count : %u, truecount = %u).",entries[i],count[i],truecount[i]);
            //PSendSysMessage("Entry : %u (%s), count : %u, truecount = %u).",entries[i],*(names[i]),count[i],truecount[i]);
...
bool ChatHandler::ReturnOrphaned(const char* args) {
    /*.returnorphaned [from] [to] [confirm]
    No confirm will only simulate the process
    */
   ...
   if (!args || !*args)
    ...
    Tokens tokens_args;
    tokens_args = StrSplit(args, " ");

    if(tokens_args.size() < 2)
        return false;

    unsigned int from = atoi(tokens_args[0].c_str());
    unsigned int to = atoi(tokens_args[1].c_str()); //last is 41563
    unsigned int confirm = 0;
    if (tokens_args.size() > 2)
        confirm = atoi(tokens_args[2].c_str());

    if(from > to || to >= LIST_SIZE)
    {
        PSendSysMessage("ReturnOrphaned : Invalid range");
        return true;
    }

    std::vector<unsigned int> ignorelist;
    ignorelist.push_back(27860); //eau de draenor
    ignorelist.push_back(0); //fin de liste pour le find
    ...
    for (int i = from; i <= to; i++)
    {
        ...
        if (!GetItemDataByGuid(tokens,itemguid[i]))
        {
            PSendSysMessage("ReturnOrphaned : Objet trouvable (guid : %u)", itemguid[i]);
            continue;
        }
        ...
        if (!proto)
        {
            PSendSysMessage("ReturnOrphaned : Proto introuvable : objet %u",itemguid[i]);
            continue;
        }

        std::vector<unsigned int>::iterator it = find(ignorelist.begin(), ignorelist.end(), proto->ItemId);
        if (it != ignorelist.end())
        {
            PSendSysMessage("ReturnOrphaned : Item with guid %u (entry %u) is in ignore list",itemguid[i],proto->ItemId);
            continue;
        }

        uint64 ownerfullguid = MAKE_NEW_GUID(ownerguid[i],0,HIGHGUID_PLAYER);
        Player *owner = objmgr.GetPlayer(ownerfullguid);
        if(!owner)
        {
            if(!objmgr.GetPlayerAccountIdByGUID(ownerguid[i])) //owner dont exists
            {
                PSendSysMessage("ReturnOrphaned : Owner %u of item %u not found",ownerguid[i],itemguid[i]);
                continue;
            }
        }
        ...
        if (ItemIsInMailOrInventory(itemguid[i]))
        {
            PSendSysMessage("ReturnOrphaned : Item %u is already in mail or inventory",itemguid[i]);
            continue;
        }
        ...
        if (confirm)
        {
            ...
            if(!item->LoadFromDB(itemguid[i],0, NULL))
            {
                delete item;
                continue;
            }
            ...
            MailItemsInfo mi;
            mi.AddItem(itemguid[i],proto->ItemId,item);

            std::ostringstream subject;
            subject << proto->ItemId << ":0:" << AUCTION_EXPIRED;
            WorldSession::SendMailTo(owner, MAIL_AUCTION, MAIL_STATIONERY_AUCTION, 7, ownerguid[i], subject.str(), 0, &mi, 0, 0, MAIL_CHECK_MASK_NONE);
            PSendSysMessage("ReturnOrphaned : Item %u (id %u, %s) sent to owner %u",itemguid[i],proto->ItemId,proto->Name1,ownerguid[i]);
        } else {
            PSendSysMessage("ReturnOrphaned (simulation) : Item %u (id %u, %s) sent to owner %u",itemguid[i],proto->ItemId,proto->Name1,ownerguid[i]);
        }
    }
