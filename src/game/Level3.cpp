...
bool ChatHandler::GetItemDataByGuid(Tokens& tokens, unsigned int guid)
{
    ...
    if(!result)
    {
       /* PSendSysMessage("SELECT data FROM item_instance WHERE guid=%u", guid);
        PSendSysMessage("ChatHandler::GetItemDataByGuid : No result."); */
        return false;
    }
    ...
    char* osef1 = strtok((char*)data, " ");
    char* osef2 = strtok(NULL, " ");
    char* osef3 = strtok(NULL, " ");
    char* c_item_entry = strtok(NULL, " ");

    PSendSysMessage("tokens.size() = %i",tokens.size());
...
bool ChatHandler::ItemIsInMail(unsigned int guid)
{
...
bool ChatHandler::ReturnOrphaned(const char* args) {
   ...
   if (!args || !*args)
    ...
    unsigned int confirm = atoi(tokens_args[2].c_str());
    ...
    for (int i = from; i <= to; i++)
    {
        ...
        Player *owner = objmgr.GetPlayer(ownerguid[i]);
        if (!owner)
        {
        ...
        if (ItemIsInMail(itemguid[i]))
        {
            PSendSysMessage("ReturnOrphaned : Item %u is already in mail",itemguid[i]);
            continue;
        }
