...
ItemPrototype const* ChatHandler::GetItemProtoByGuid(unsigned int guid)
{
    ...
    if(!result)
    {
        PSendSysMessage("ChatHandler::GetItemProtoByGuid : No result.");
        return NULL;
    }
    ...
    if (!fields) 
    {
        PSendSysMessage("ChatHandler::GetItemProtoByGuid : Empty set.");
        return NULL;
    }
    ...
    if(tokens.size() < 4)
    {
        PSendSysMessage("ChatHandler::GetItemProtoByGuid : Lolwtf. Incorrect data size.");
        return NULL;
    }

    unsigned item_entry = atoi(tokens[3].c_str());
    // PSendSysMessage("GetItemProtoByGuid : Found item_entry : %u",item_entry);

    return objmgr.GetItemPrototype(item_entry);
}

unsigned int ChatHandler::GetItemCountByGuid(unsigned int guid)
{
    QueryResult* result = CharacterDatabase.PQuery("SELECT data FROM item_instance WHERE guid=%u", guid);

    if(!result)
    {
        PSendSysMessage("ChatHandler::GetItemCountByGuid : No result.");
        return NULL;
    }
    ...
    if (!fields) {
        PSendSysMessage("ChatHandler::GetItemCountByGuid : Empty set.");
        return NULL;
    }
    ...
    if(tokens.size() < 15)
    {
        PSendSysMessage("ChatHandler::GetItemCountByGuid : Lolwtf. Incorrect data size.");
        return NULL;
    }

    unsigned count = atoi(tokens[14].c_str());
    //PSendSysMessage("GetItemCountByGuid : Found count : %u",count);
...
bool ChatHandler::HandleListOrphanedFor(const char* args) {
    ...
    for (int i = startindex; i < LIST_SIZE && ownerguid[i] == playerguid; i++)
    {
        if (ItemPrototype const* proto = GetItemProtoByGuid(itemguid[i])) 
            PSendSysMessage("guid : %u - id : %u (%s) x %u",itemguid[i], proto->ItemId, proto->Name1,GetItemCountByGuid(itemguid[i]));
         else
...
bool ChatHandler::HandleListMostOrphaned(const char* args) {
    ...
    for(int i = 0; i < LIST_SIZE; i++)
    {
        if (ItemPrototype const* proto = GetItemProtoByGuid(itemguid[i]))
        {
            ...
            if(unsigned int index = std::find(entries.begin(), entries.end(), entry) == entries.end())
            {
                ...
                count.push_back(1);
                truecount.push_back(GetItemCountByGuid(itemguid[i]));
            } else {
                entries[i] = entry;
                count[i] = count[i] + 1;
                truecount[i] = truecount[i] + GetItemCountByGuid(itemguid[i]);
            }
        } else {
            PSendSysMessage("Erreur : Objet trouvable (guid : %u)", itemguid[i]);
        }
    }
    ...
    for (int i = 0; i < count.size(); i++)
    {
        ...
        {
            if (ItemPrototype const* proto = GetItemProtoByGuid(itemguid[i]))
                PSendSysMessage("Entry : %u (%s), count : %u, truecount = %ux).",entries[i],proto->Name1,count[i],truecount[i]);
             else 
                PSendSysMessage("Entry : %u, count : %u, truecount = %ux).",entries[i],count[i],truecount[i]);
        }
    }
