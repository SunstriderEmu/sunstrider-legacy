...
bool GameObject::Create(uint32 guidlow, uint32 name_id, Map *map, float x, float y, float z, float ang, float rotation0, float rotation1, float rotation2, float rotation3, uint32 animprogress, uint32 go_state, uint32 ArtKit)
{
    ...
    GameObjectTemplate const* goinfo = objmgr.GetGameObjectTemplate(name_id);
    if (!goinfo)
    {
            ...
            if(goInfo->type == GAMEOBJECT_TYPE_TRAP)
            {
                ...
                if(!radius)
                {
                    if(goInfo->trap.cooldown != 3)            // cast in other case (at some triggering/linked go/etc explicit call)
                    {
                        ...
                        if(const SpellEntry *spellEntry = spellmgr.LookupSpell(goInfo->trap.spellId))
...
void GameObject::TriggeringLinkedGameObject( uint32 trapEntry, Unit* target)
{
    ...
    SpellEntry const* trapSpell = spellmgr.LookupSpell(trapInfo->trap.spellId);
...
void GameObject::GetRespawnPosition(float &x, float &y, float &z, float* ori /* = NULL*/) const
{
    if (m_DBTableGuid)
    {
        if (GameObjectData const* data = objmgr.GetGOData(GetDBTableGUIDLow()))
        {
            x = data->posX;
            y = data->posY;
            z = data->posZ;
