...
void Unit::RemoveMovementImpairingAuras()
{
    ...
    {
        if(sSpellMgr->GetSpellCustomAttr(iter->second->GetId()) & SPELL_ATTR0_CU_MOVEMENT_IMPAIR)
            RemoveAura(iter);
        else
            ++iter;
    }
...
bool Unit::HasAuraTypeWithFamilyFlags(AuraType auraType, uint32 familyName,uint64 familyFlags) const
{
    ...
    for(AuraList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        if(SpellEntry const *iterSpellProto = (*itr)->GetSpellInfo())
...
{
    if (sSpellMgr->GetSpellCustomAttr(spell) & SPELL_ATTR0_CU_CANT_BREAK_CC)
        return;
...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    else                                                    // if (health <= damage)
    {
        ...
        {
            /*const SpellEntry *se = i->second->GetSpellInfo();
            next = i; ++next;
...
void Unit::CalculateSpellDamageTaken(SpellNonMeleeDamage *damageInfo, int32 damage, SpellEntry const *spellInfo, WeaponAttackType attackType, bool crit)
{
    ...
    switch (spellInfo->DmgClass)
    {
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (crit)
            {
                ...
                // Resilience - reduce crit damage
                if (pVictim->GetTypeId()==TYPEID_PLAYER && !(spellInfo->AttributesEx4 & SPELL_ATTR4_IGNORE_RESISTANCES))
    ...
    if( damageSchoolMask & SPELL_SCHOOL_MASK_NORMAL  && (sSpellMgr->GetSpellCustomAttr(spellInfo->Id) & SPELL_ATTR0_CU_IGNORE_ARMOR) == 0)
...
void Unit::DealSpellDamage(SpellNonMeleeDamage *damageInfo, bool durabilityLoss)
{
    ...
    SpellEntry const *spellProto = sSpellMgr->GetSpellInfo(damageInfo->SpellID);
    if (spellProto == NULL)
    {
    ...
    if(damageInfo->damage && spellProto->Id == 35395)
    {
        ...
        for(AuraMap::iterator itr = vAuras.begin(); itr != vAuras.end(); ++itr)
        {
            ...
            if(spellInfo->SpellFamilyName == SPELLFAMILY_PALADIN && spellInfo->AttributesEx3 & SPELL_ATTR3_CANT_MISS)
            {
    ...
    if (damageInfo->procVictim & PROC_FLAG_TAKEN_ANY_DAMAGE)
    {
        ...
        for(AuraList::const_iterator i = vDamageShields.begin(), next = vDamageShields.begin(); i != vDamageShields.end(); i = next)
        {
           ...
           if (alreadyDone.find(*i) == alreadyDone.end())
           {
               ...
               SpellEntry const *spellProto = sSpellMgr->GetSpellInfo((*i)->GetId());
               if(!spellProto)
                   continue;
...
void Unit::CalcAbsorbResist(Unit *pVictim,SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32 *absorb, uint32 *resist, uint32 spellId)
{
    ...
    if(  (schoolMask & SPELL_SCHOOL_MASK_SPELL)                                          // Is magic and not holy
         && (  !spellProto 
               ...
               || !(spellProto->AttributesEx3 & SPELL_ATTR3_CANT_MISS) ) // Non binary spell (this was already handled in DoSpellHitOnUnit) (see Spell::IsBinaryMagicResistanceSpell for more)
      )              
    {
    ...
    for(AuraList::const_iterator i = vManaShield.begin(), next; i != vManaShield.end() && RemainingDamage > 0; i = next)
    {
        ...
        float manaMultiplier = (*i)->GetSpellInfo()->EffectValueMultiplier[(*i)->GetEffIndex()];
        if(Player *modOwner = GetSpellModOwner())
...
SpellMissInfo Unit::MagicSpellHitResult(Unit *pVictim, SpellEntry const *spell, Item* castItem)
{
    ...
    if (!pVictim->IsAlive() || spell->AttributesEx3 & SPELL_ATTR3_CANT_MISS)
        return SPELL_MISS_NONE;
...
SpellMissInfo Unit::SpellHitResult(Unit *pVictim, SpellEntry const *spell, bool CanReflect, Item* castItem)
{
    ...
    if (CanReflect)
    {
        ...
        for(Unit::AuraList::const_iterator i = mReflectSpellsSchool.begin(); i != mReflectSpellsSchool.end(); ++i) {
            ...
            if((*i)->GetModifier()->m_miscvalue & GetSpellSchoolMask(spell)) {
                //TC_LOG_INFO("For2 %u %u %u %u %d", spell->Id, (*i)->GetId(), (*i)->GetModifier()->m_miscvalue, GetSpellSchoolMask(spell), (*i)->GetModifierValue());
    ...
    if(    sSpellMgr->IsBinaryMagicResistanceSpell(spell)
        && !(spell->AttributesEx4 & SPELL_ATTR4_IGNORE_RESISTANCES) 
        && !(spell->AttributesEx3 & SPELL_ATTR3_CANT_MISS)  )
    {
...
bool Unit::AddAura(Aura *Aur)
{
    ...
    SpellEntry const* aurSpellInfo = Aur->GetSpellInfo();
    ...
    if (!Aur->IsPassive() && !Aur->IsPersistent())
    {
        for(AuraMap::iterator i2 = m_Auras.lower_bound(spair); i2 != m_Auras.upper_bound(spair);)
        {
            ...
            else if (sSpellMgr->GetSpellCustomAttr(Aur->GetId()) & SPELL_ATTR0_CU_SAME_STACK_DIFF_CASTERS) {
                stackModified=true;
                Aur->SetStackAmount(i2->second->GetStackAmount());
                if(Aur->GetStackAmount() < aurSpellInfo->StackAmount)
                    Aur->SetStackAmount(Aur->GetStackAmount()+1);
            }
            else if (sSpellMgr->GetSpellCustomAttr(Aur->GetId()) & SPELL_ATTR0_CU_ONE_STACK_PER_CASTER_SPECIAL) {
                ++i2;
                continue;
            }
    ...
    if (Aur->GetModifier()->m_auraname < TOTAL_AURAS)
    {
        ...
        if (AuraStateType aState = sSpellMgr->GetSpellAuraState(Aur->GetSpellInfo()))
    ...
    uint32 id = Aur->GetId();
    if(sSpellMgr->GetSpellCustomAttr(id) & SPELL_ATTR0_CU_LINK_AURA)
    {
...
bool Unit::RemoveNoStackAurasDueToAura(Aura *Aur)
{
    ...
    SpellEntry const* spellProto = Aur->GetSpellInfo();
    if (!spellProto)
        return false;
    ...
    for (i = m_Auras.begin(); i != m_Auras.end(); i = next)
    {
        ...
        SpellEntry const* i_spellProto = (*i).second->GetSpellInfo();
...
void Unit::RemoveAurasWithAttribute(uint32 flags)
{
    for (AuraMap::iterator iter = m_Auras.begin(); iter != m_Auras.end();)
    {
        SpellEntry const *spell = iter->second->GetSpellInfo();
...
void Unit::RemoveAurasWithCustomAttribute(uint32 flags)
{
    for (AuraMap::iterator iter = m_Auras.begin(); iter != m_Auras.end();) {
        SpellEntry const *spell = iter->second->GetSpellInfo();
        if (sSpellMgr->GetSpellCustomAttr(spell->Id) & flags)
            RemoveAura(iter);
        else
            ++iter;
    }
...
void Unit::RemoveAurasWithDispelType( DispelType type )
{
    ...
    {
        SpellEntry const* spell = itr->second->GetSpellInfo();
...
void Unit::RemoveAura(AuraMap::iterator &i, AuraRemoveMode mode)
{
    ...
    AuraStateType auraState = sSpellMgr->GetSpellAuraState(Aur->GetSpellInfo());
    if (auraState)
    {
        bool canBreak = false;
    ...
    if (!stack)
    {
        ...
        // Remove Linked Auras
        uint32 id = Aur->GetId();
        if(sSpellMgr->GetSpellCustomAttr(id) & SPELL_ATTR0_CU_LINK_REMOVE)
        {
...
void Unit::RemoveArenaAuras(bool onleave)
{
    ...
    for(AuraMap::iterator iter = m_Auras.begin(); iter != m_Auras.end();)
    {
        if  (  !(iter->second->GetSpellInfo()->AttributesEx4 & SPELL_ATTR4_UNK21) // don't remove stances, shadowform, pally/hunter auras
            ...
            && (iter->second->IsPositive() || !(iter->second->GetSpellInfo()->AttributesEx3 & SPELL_ATTR3_DEATH_PERSISTENT)) //dont remove death persistent debuff such as deserter
            )
            RemoveAura(iter);
        else
            ++iter;
...
void Unit::RemoveGameObject(GameObject* gameObj, bool del)
{
    ...
    {
        SpellEntry const* createBySpell = sSpellMgr->GetSpellInfo(gameObj->GetSpellId());
...
{
    SpellEntry const *hasteSpell = triggeredByAura->GetSpellInfo();
    ...
    SpellEntry const* triggerEntry = sSpellMgr->GetSpellInfo(triggered_spell_id);
...
{
    SpellEntry const *dummySpell = triggeredByAura->GetSpellInfo();
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                ...
                case 28764:
                {
                    ...
                    {
                        if(SpellEntry const* iterSpellProto = (*iter)->GetSpellInfo())
                        {
        ...
        case SPELLFAMILY_SHAMAN:
        {
            switch(dummySpell->Id)
            {
                ...
                case 33757:
                {
                    ...
                    SpellEntry const* windfurySpellEntry = sSpellMgr->GetSpellInfo(spellId);
...
bool Unit::HandleProcTriggerSpell(Unit *pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 ProcFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->SpellFamilyName)
    {
     ...
     case SPELLFAMILY_WARLOCK:
     {
         ...
         else if (auraSpellInfo->SpellFamilyFlags & 0x0000000000004000LL)
         {
             ...
             for(Unit::AuraList::const_iterator i = mAddFlatModifier.begin(); i != mAddFlatModifier.end(); ++i)
             {
                 ...
                 {
                     int32 value2 = CalculateSpellDamage((*i)->GetSpellInfo(),2,(*i)->GetSpellInfo()->EffectBasePoints[2],this);
     ...
     case SPELLFAMILY_PALADIN:
     {
         ...
         else if (auraSpellInfo->SpellIconID==241)
         {
             ...
             SpellEntry const *originalSpell = sSpellMgr->GetSpellInfo(originalSpellId);
             if(!originalSpell)
             {
    ...
    SpellEntry const* triggerEntry = sSpellMgr->GetSpellInfo(trigger_spell_id);
...
bool Unit::HandleOverrideClassScriptAuraProc(Unit *pVictim, Aura *triggeredByAura, SpellEntry const *procSpell, uint32 cooldown)
{
    ...
    switch(scriptId)
    {
        ...
        {
            int32 chance = triggeredByAura->GetSpellInfo()->EffectBasePoints[triggeredByAura->GetEffIndex()];
            if(!roll_chance_i(chance))
                return false;
...
void Unit::ModifyAuraState(AuraStateType flag, bool apply)
{
    ...
    else
    {
        if (HasFlag(UNIT_FIELD_AURASTATE,1<<(flag-1)))
        {
            ...
            {
                SpellEntry const* spellProto = (*itr).second->GetSpellInfo();
...
int32 Unit::SpellBaseDamageBonus(SpellSchoolMask schoolMask, Unit* pVictim)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for(AuraList::const_iterator i = mDamageDoneOfStatPercent.begin();i != mDamageDoneOfStatPercent.end(); ++i)
        {
            ...
            {
                SpellEntry const* iSpellProto = (*i)->GetSpellInfo();
...
void Unit::ApplySpellHealingTargetModifiers(SpellEntry const *spellProto, DamageEffectType /* damageType */, int& healpower, float& healcoef, int32& flathealbonus, Unit *pVictim)
{
    ...
    if (spellProto->SpellFamilyName == SPELLFAMILY_PALADIN && (spellProto->SpellFamilyFlags & 0x00000000C0000000LL))
    {
        ...
        for(AuraList::const_iterator i = mDummyAuras.begin();i != mDummyAuras.end(); ++i)
        {
            if((*i)->GetSpellInfo()->SpellVisual == 9180)
            {
                // Flash of Light
...
int32 Unit::SpellBaseHealingBonus(SpellSchoolMask schoolMask)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for(AuraList::const_iterator i = mHealingDoneOfStatPercent.begin();i != mHealingDoneOfStatPercent.end(); ++i)
        {
            ...
            Stats usedStat = Stats((*i)->GetSpellInfo()->EffectMiscValue[(*i)->GetEffIndex()]);
...
void Unit::ModSpellCastTime(SpellEntry const* spellProto, int32 & castTime, Spell * spell)
{
    ...
    if(Player* modOwner = GetSpellModOwner())
     ...
     if (spellProto->Attributes & SPELL_ATTR0_RANGED && !(spellProto->AttributesEx2 & SPELL_ATTR2_AUTOREPEAT_FLAG))
            castTime = int32 (float(castTime) * m_modAttackSpeedPct[RANGED_ATTACK]);
     else 
        if(spellProto->SpellFamilyName || (spellProto->AttributesEx5 & SPELL_ATTR5_HASTE_AFFECT_DURATION) )
...
bool Unit::IsFrozen() const
{
    ...
    for(AuraList::const_iterator i = mRoot.begin(); i != mRoot.end(); ++i)
        if( GetSpellSchoolMask((*i)->GetSpellInfo()) & SPELL_SCHOOL_MASK_FROST)
            return true;
    return false;
}
...
//                    TC_LOG_DEBUG("FIXME","     <end of list>");
        ...
        SpellEntry const *spellInfo = triggeredByAura->GetSpellInfo();
        uint32 effIndex = triggeredByAura->GetEffIndex();
        ...
        switch(auraModifier->m_auraname)
        {
            ...
            case SPELL_AURA_MOD_HASTE:
            {
                ...
                if (triggeredByAura->GetSpellInfo()->SpellVisual == 2759 && triggeredByAura->GetSpellInfo()->SpellIconID == 108) { // Shaman and Warrior Flurry
                    if (procExtra & PROC_EX_CRITICAL_HIT)
                        useCharges = false;
                }
...
{
    SpellEntry const* spellProto = aura->GetSpellInfo();
...
bool Unit::HasAuraWithMechanic(Mechanics mechanic) const
{
    ...
    for(AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        if(SpellEntry const *iterSpellProto = itr->second->GetSpellInfo())
