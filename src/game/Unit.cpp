...
void Unit::RemoveAurasWithInterruptFlags(uint32 flag, uint32 except, bool withChanneled)
{
    ...
    for (iter = m_interruptableAuras.begin(); iter != m_interruptableAuras.end(); )
    {
        ...
        //TC_LOG_DEBUG("FIXME","auraflag:%u flag:%u = %u", aur->GetSpellProto()->AuraInterruptFlags,flag, aur->GetSpellProto()->AuraInterruptFlags & flag);
...
bool Unit::HasAuraTypeWithFamilyFlags(AuraType auraType, uint32 familyName,uint64 familyFlags) const
{
    ...
    for(AuraList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        if(SpellInfo const *iterSpellProto = (*itr)->GetSpellProto())
...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellInfo const *spellProto, bool durabilityLoss)
{
    ...
    else                                                    // if (health <= damage)
    {
        ...
        {
            /*const SpellInfo *se = i->second->GetSpellProto();
            next = i; ++next;
...
void Unit::DealSpellDamage(SpellNonMeleeDamage *damageInfo, bool durabilityLoss)
{
    ...
    if(damageInfo->damage && spellProto->Id == 35395)
    {
        ...
        {
            SpellInfo const *spellInfo = (*itr).second->GetSpellProto();
...
bool Unit::AddAura(Aura *Aur)
{
    ...
    if( (!IsAlive() && !(Aur->GetSpellProto()->Attributes & SPELL_ATTR0_CASTABLE_WHILE_DEAD)) && Aur->GetId() != 20584 && Aur->GetId() != 8326 && Aur->GetId() != 2584 &&
    ...
    if ((Aur->DoesAuraApplyAuraName(SPELL_AURA_MOD_CONFUSE) || Aur->DoesAuraApplyAuraName(SPELL_AURA_MOD_CHARM) ||
        Aur->DoesAuraApplyAuraName(SPELL_AURA_MOD_STUN)) && (Aur->GetSpellProto()->Attributes & SPELL_ATTR0_BREAKABLE_BY_DAMAGE))
        m_justCCed = 2;

    SpellInfo const* aurSpellInfo = Aur->GetSpellProto();
    ...
    if (Aur->GetModifier()->m_auraname < TOTAL_AURAS)
    {
        ...
        if((Aur->GetSpellProto()->Attributes & SPELL_ATTR0_BREAKABLE_BY_DAMAGE)
...
bool Unit::RemoveNoStackAurasDueToAura(Aura *Aur)
{
    ...
    SpellInfo const* spellProto = Aur->GetSpellProto();
    if (!spellProto)
        return false;
    ...
    for (i = m_Auras.begin(); i != m_Auras.end(); i = next)
    {
        ...
        SpellInfo const* i_spellProto = (*i).second->GetSpellProto();
...
void Unit::RemoveArenaAuras(bool onleave)
{
    ...
    for(AuraMap::iterator iter = m_Auras.begin(); iter != m_Auras.end();)
    {
        if  (  !(iter->second->GetSpellProto()->AttributesEx4 & SPELL_ATTR4_UNK21) // don't remove stances, shadowform, pally/hunter auras
            && !iter->second->IsPassive()                               // don't remove passive auras
            && (!(iter->second->GetSpellProto()->Attributes & SPELL_ATTR2_PRESERVE_ENCHANT_IN_ARENA))
            && (!onleave || !iter->second->IsPositive())                // remove all buffs on enter, negative buffs on leave
            && (iter->second->IsPositive() || !(iter->second->GetSpellProto()->AttributesEx3 & SPELL_ATTR3_DEATH_PERSISTENT)) //dont remove death persistent debuff such as deserter
            )
            RemoveAura(iter);
        else
            ++iter;
...
{
    SpellInfo const *hasteSpell = triggeredByAura->GetSpellProto();
...
{
    SpellInfo const *dummySpell = triggeredByAura->GetSpellProto ();
        ...
        case SPELLFAMILY_GENERIC:
        {
            switch (dummySpell->Id)
            {
                ...
                case 28764:
                {
                    ...
                    {
                        if(SpellInfo const* iterSpellProto = (*iter)->GetSpellProto())
                        {
...
bool Unit::HandleProcTriggerSpell(Unit *pVictim, uint32 damage, Aura* triggeredByAura, SpellInfo const *procSpell, uint32 ProcFlags, uint32 procEx, uint32 cooldown)
{
    ...
    SpellInfo const* auraSpellInfo = triggeredByAura->GetSpellProto();
...
uint32 Unit::SpellDamageBonus(Unit *pVictim, SpellInfo const *spellProto, uint32 pdamage, DamageEffectType damagetype)
{
    ...
    for(AuraList::const_iterator i = mModDamagePercentDone.begin(); i != mModDamagePercentDone.end(); ++i)
    {
        ...
        if((*i)->GetSpellProto()->Attributes & SPELL_ATTR0_AFFECT_WEAPON && (*i)->GetSpellProto()->EquippedItemClass != -1) 
            continue;
...
int32 Unit::SpellBaseDamageBonus(SpellSchoolMask schoolMask, Unit* pVictim)
{
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
        ...
        for(AuraList::const_iterator i = mDamageDoneOfStatPercent.begin();i != mDamageDoneOfStatPercent.end(); ++i)
        {
            ...
            {
                SpellInfo const* iSpellProto = (*i)->GetSpellProto();
...
//                    TC_LOG_DEBUG("FIXME","     <end of list>");
        ...
        SpellInfo const *spellInfo = triggeredByAura->GetSpellProto();
        uint32 effIndex = triggeredByAura->GetEffIndex();
...
{
    SpellInfo const* spellProto = aura->GetSpellProto ();
...
bool Unit::HandleMendingAuraProc( Aura* triggeredByAura )
{
    ...
    SpellInfo const* spellProto = triggeredByAura->GetSpellProto();
...
bool Unit::HasAuraWithMechanic(Mechanics mechanic) const
{
    ...
    for(AuraMap::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
        if(SpellInfo const *iterSpellProto = itr->second->GetSpellProto())
