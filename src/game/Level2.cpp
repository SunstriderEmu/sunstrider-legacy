...
bool ChatHandler::HandleNpcAddFormationCommand(const char* args)
{
    ...
    WorldDatabase.PExecuteLog("INSERT INTO `creature_formations` (`leaderGUID`, `memberGUID`, `dist_min`, `angle`, `groupAI`) VALUES ('%u','%u','%f', '%f', '%u')",
...
bool ChatHandler::HandleChanBan(const char* args)
{
    ...
    LogsDatabase.PExecute("INSERT INTO sanctions VALUES (%u, %u, %u, %u, "I64FMTD", '%s'", accountid, m_session->GetPlayer()->GetGUIDLow(), uint32(SANCTION_CHANBAN), durationSecs, time(NULL), reasonstr.c_str());
