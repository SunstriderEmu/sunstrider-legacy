...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellEntry* spell, GameObject* gob)
{
    ...
    if (Unit* tempInvoker = GetLastInvoker())
        TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: Invoker: %s (guidlow: %u)", tempInvoker->GetName(), tempInvoker->GetGUIDLow());
    ...
    switch (e.GetActionType())
    {
        case SMART_ACTION_TALK:
        {
            ...
            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_TALK: talker: %s (GuidLow: %u), textGuid: %u",
        ...
        case SMART_ACTION_SIMPLE_TALK:
        {
            ...
            if (targets)
            {
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_SIMPLE_TALK: talker: %s (GuidLow: %u), textGroupId: %u",
            ...
            if (targets)
            {
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    if (IsUnit(*itr))
                    {
                        ...
                        TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_PLAY_EMOTE: target: %s (GuidLow: %u), emote: %u",
            ...
            if (targets)
            {
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    if (IsUnit(*itr))
                    {
                        ...
                        TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_SOUND: target: %s (GuidLow: %u), sound: %u, onlyself: %u",
                ...
                for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
                {
                    if (IsCreature(*itr))
                    {
                        if (e.action.faction.factionID)
                        {
                            ...
                            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_SET_FACTION: Creature entry %u, GuidLow %u set faction to %u",
        ...
        case SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
                {
                    ...
                    if (e.action.morphOrMount.creature)
                    {
                        if (CreatureTemplate const* ci = GetCreatureTemplateStore(e.action.morphOrMount.creature))
                        {
                            ...
                            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL: Creature entry %u, GuidLow %u set displayid to %u",
                ...
                else
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL: Creature entry %u, GuidLow %u demorphs.",
        ...
        case SMART_ACTION_FAIL_QUEST:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsPlayer(*itr))
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_FAIL_QUEST: Player guidLow %u fails quest %u",
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsPlayer(*itr))
                {
                    if (Quest const* q = sObjectMgr->GetQuestTemplate(e.action.quest.quest))
                    {
                        ...
                        TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_ADD_QUEST: Player guidLow %u add quest %u",
                            (*itr)->GetGUIDLow(), e.action.quest.quest);
                    }
                }
            }
        ...
        case SMART_ACTION_RANDOM_EMOTE:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_RANDOM_EMOTE: Creature guidLow %u handle random emote %u",
        ...
        case SMART_ACTION_THREAT_SINGLE_PCT:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_THREAT_SINGLE_PCT: Creature guidLow %u modify threat for unit %u, value %i",
        ...
        case SMART_ACTION_CALL_AREAEXPLOREDOREVENTHAPPENS:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsPlayer(*itr))
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_CALL_AREAEXPLOREDOREVENTHAPPENS: Player guidLow %u credited quest %u",
                        (*itr)->GetGUIDLow(), e.action.quest.quest);
                }
            }
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*itr)->ToUnit()->HasAura(e.action.cast.spell))
                {
                    ...
                    if (e.action.cast.flags & SMARTCAST_COMBAT_MOVE)
                    {
                        ...
                        SpellEntry const* spellInfo = sSpellMgr->GetSpellInfo(e.action.cast.spell);
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_CAST:: Creature %u casts spell %u on target %u with castflags %u",
                        me->GetGUIDLow(), e.action.cast.spell, (*itr)->GetGUIDLow(), e.action.cast.flags);
                }
                else
                    TC_LOG_DEBUG("FIXME","Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (Guid: " UI64FMTD " Entry: %u Type: %u) already has the aura", e.action.cast.spell, (*itr)->GetGUID(), (*itr)->GetEntry(), uint32((*itr)->GetTypeId()));
            }
        ...
        case SMART_ACTION_INVOKER_CAST:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*itr)->ToUnit()->HasAura(e.action.cast.spell))
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_INVOKER_CAST: Invoker %u casts spell %u on target %u with castflags %u",
        ...
        case SMART_ACTION_ADD_AURA:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_ADD_AURA: Adding aura %u to unit %u",
        ...
        case SMART_ACTION_ACTIVATE_GOBJECT:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsGameObject(*itr))
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_ACTIVATE_GOBJECT. Gameobject %u (entry: %u) activated",
        ...
        case SMART_ACTION_RESET_GOBJECT:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsGameObject(*itr))
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_RESET_GOBJECT. Gameobject %u (entry: %u) reset",
                        (*itr)->GetGUIDLow(), (*itr)->GetEntry());
                }
            }
        ...
        case SMART_ACTION_SET_EMOTE_STATE:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_SET_EMOTE_STATE. Unit %u set emotestate to %u",
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    if (!e.action.unitFlag.type)
                    {
                        ...
                        TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_SET_UNIT_FLAG. Unit %u added flag %u to UNIT_FIELD_FLAGS",
                        (*itr)->GetGUIDLow(), e.action.unitFlag.flag);
                    }
                    else
                    {
                        (*itr)->ToUnit()->SetFlag(UNIT_FIELD_FLAGS_2, e.action.unitFlag.flag);
                        TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_SET_UNIT_FLAG. Unit %u added flag %u to UNIT_FIELD_FLAGS_2",
                        (*itr)->GetGUIDLow(), e.action.unitFlag.flag);
                    }
                }
            }
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    if (!e.action.unitFlag.type)
                    {
                        ...
                        TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_REMOVE_UNIT_FLAG. Unit %u removed flag %u to UNIT_FIELD_FLAGS",
                        (*itr)->GetGUIDLow(), e.action.unitFlag.flag);
                    }
                    else
                    {
                        (*itr)->ToUnit()->RemoveFlag(UNIT_FIELD_FLAGS_2, e.action.unitFlag.flag);
                        TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_REMOVE_UNIT_FLAG. Unit %u removed flag %u to UNIT_FIELD_FLAGS_2",
                        (*itr)->GetGUIDLow(), e.action.unitFlag.flag);
                    }
                }
            }
        ...
        case SMART_ACTION_AUTO_ATTACK:
        {
            ...
            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_AUTO_ATTACK: Creature: %u bool on = %u",
        ...
        case SMART_ACTION_ALLOW_COMBAT_MOVEMENT:
        {
            ...
            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_ALLOW_COMBAT_MOVEMENT: Creature %u bool on = %u",
        ...
        case SMART_ACTION_SET_EVENT_PHASE:
        {
            ...
            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_SET_EVENT_PHASE: Creature %u set event phase %u",
        ...
        case SMART_ACTION_INC_EVENT_PHASE:
        {
            ...
            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_INC_EVENT_PHASE: Creature %u inc event phase by %u, "
        ...
        case SMART_ACTION_EVADE:
        {
            ...
            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_EVADE: Creature %u EnterEvadeMode", me->GetGUIDLow());
            break;
        }
        case SMART_ACTION_FLEE_FOR_ASSIST: //FIXME : Emote
        {
            if (!me)
                break;
            ...
            */
            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_FLEE_FOR_ASSIST: Creature %u DoFleeToGetAssistance", me->GetGUIDLow());
            break;
        }
        case SMART_ACTION_CALL_GROUPEVENTHAPPENS:
        {
            ...
            if (IsPlayer(unit) && GetBaseObject())
            {
                ...
                TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_CALL_GROUPEVENTHAPPENS: Player %u, group credit for quest %u",
        ...
        case SMART_ACTION_REMOVEAURASFROMSPELL:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_REMOVEAURASFROMSPELL: Unit %u, spell %u",
        ...
        case SMART_ACTION_FOLLOW:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsUnit(*itr))
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_FOLLOW: Creature %u following target %u",
        ...
        case SMART_ACTION_RANDOM_PHASE:
        {
            ...
            SetPhase(phase);
            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_RANDOM_PHASE: Creature %u sets event phase to %u",
        ...
        case SMART_ACTION_RANDOM_PHASE_RANGE:
        {
            ...
            SetPhase(phase);
            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_RANDOM_PHASE_RANGE: Creature %u sets event phase to %u",
                GetBaseObject()->GetGUIDLow(), phase);
            break;
        }
        case SMART_ACTION_CALL_KILLEDMONSTER:
        {
            if (e.target.type == SMART_TARGET_NONE || e.target.type == SMART_TARGET_SELF) // Loot recipient and his group members
            {
                ...
                if (Player* player = me->GetLootRecipient())
                {
                    ...
                    TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_CALL_KILLEDMONSTER: Player %u, Killcredit: %u",
        ...
        case SMART_ACTION_SET_INST_DATA:
        {
            ...
            if (!instance)
            {
                TC_LOG_ERROR("FIXME","SmartScript: Event %u attempt to set instance data without instance script. EntryOrGuid %d", e.GetEventType(), e.entryOrGuid);
                break;
            }

            instance->SetData(e.action.setInstanceData.field, e.action.setInstanceData.data);
            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_SET_INST_DATA: Field: %u, data: %u",
        ...
        case SMART_ACTION_SET_INST_DATA64:
        {
            ...
            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_SET_INST_DATA64: Field: %u, data: " UI64FMTD,
        ...
        case SMART_ACTION_DIE:
        {
            ...
            {
                me->Kill(me);
                TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_DIE: Creature %u", me->GetGUIDLow());
            }
            break;
        }
        case SMART_ACTION_SET_IN_COMBAT_WITH_ZONE:
        {
            if (me)
            {
                ...
                TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_SET_IN_COMBAT_WITH_ZONE: Creature %u", me->GetGUIDLow());
            }
            break;
        }
        /*
        case SMART_ACTION_CALL_FOR_HELP:
        {
            if (me)
            {
                ...
                TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_CALL_FOR_HELP: Creature %u", me->GetGUIDLow());
            }
            break;
        }
        */
        ...
        case SMART_ACTION_SET_SHEATH:
        {
            if (me)
            {
                ...
                TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction: SMART_ACTION_SET_SHEATH: Creature %u, State: %u",
        ...
        */
                ...
                if (Creature* npc = (*itr)->ToCreature())
                {
                   ...
                   /* int8 equipId = (int8)e.action.equip.entry;
                    if (equipId)
                    {
                        ...
                        if (!einfo)
                        {
                            TC_LOG_ERROR("FIXME","SmartScript: SMART_ACTION_EQUIP uses non-existent equipment info id %u for creature %u", equipId, npc->GetEntry());
                            break;
                        }
        ...
        case SMART_ACTION_CALL_TIMED_ACTIONLIST:
        {
            ...
            {
                TC_LOG_ERROR("FIXME","SmartScript: Entry %d SourceType %u Event %u Action %u is using TARGET_NONE(0) for Script9 target. Please correct target_type in database.", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
                break;
            }
            ...
            for (ObjectList::const_iterator itr = casters->begin(); itr != casters->end(); ++itr)
            {
                ...
                for (ObjectList::const_iterator it = targets->begin(); it != targets->end(); ++it)
                {
                    ...
                    if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*it)->ToUnit()->HasAura(e.action.cast.spell))
                    {
                        ...
                        TC_LOG_DEBUG("FIXME","Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (Guid: " UI64FMTD " Entry: %u Type: %u) already has the aura", e.action.cast.spell, (*it)->GetGUID(), (*it)->GetEntry(), uint32((*it)->GetTypeId()));
                }
            }
        ...
        case SMART_ACTION_SEND_GOSSIP_MENU: //FIXME
        {
            if (!GetBaseObject())
                break;

            TC_LOG_DEBUG("FIXME","SmartScript::ProcessAction:: SMART_ACTION_SEND_GOSSIP_MENU: gossipMenuId %d, gossipNpcTextId %d",
        ...
        default:
            TC_LOG_ERROR("FIXME","SmartScript::ProcessAction: Entry %d SourceType %u, Event %u, Unhandled Action type %u", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            break;
    }
    ...
    if (e.link && e.link != e.event_id)
    {
        ...
        if (linked.GetActionType() && linked.GetEventType() == SMART_EVENT_LINK)
            ...
            TC_LOG_ERROR("FIXME","SmartScript::ProcessAction: Entry %d SourceType %u, Event %u, Link Event %u not found or invalid, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.link);
    }
}
...
void SmartScript::InstallTemplate(SmartScriptHolder const& e)
{
    ...
    if (mTemplate)
    {
        TC_LOG_ERROR("FIXME","SmartScript::InstallTemplate: Entry %d SourceType %u AI Template can not be set more then once, skipped.", e.entryOrGuid, e.GetScriptType());
        return;
    }
    ...
    switch (e.GetTargetType())
    {
        ...
        case SMART_TARGET_GAMEOBJECT_RANGE:
        {
            ...
            for (ObjectList::const_iterator itr = units->begin(); itr != units->end(); ++itr)
            {
                ...
                if (((e.target.goRange.entry && IsGameObject(*itr) && (*itr)->ToGameObject()->GetEntry() == e.target.goRange.entry) || !e.target.goRange.entry) && baseObject->IsWithinDistInMap((*itr), (float)e.target.goRange.minDist, (float)e.target.goRange.maxDist))
                    l->push_back(*itr);
            }
        ...
        case SMART_TARGET_PLAYER_RANGE:
        {
            ...
            if (!units->empty() && baseObject)
                for (ObjectList::const_iterator itr = units->begin(); itr != units->end(); ++itr)
                    if (IsPlayer(*itr) && baseObject->IsWithinDistInMap(*itr, (float)e.target.playerRange.minDist, (float)e.target.playerRange.maxDist))
                        l->push_back(*itr);
        ...
        case SMART_TARGET_THREAT_LIST:
        {
            if (me)
            {
                ...
                for (auto itr : threatList)
                    if (Unit* temp = Unit::GetUnit(*me, itr->getUnitGuid()))
                        l->push_back(temp);
            }
            break;
        }
...
void SmartScript::ProcessEvent(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, const SpellEntry* spell, GameObject* gob)
{
    ...
    switch (e.GetEventType())
    {
        ...
        case SMART_EVENT_RANGE:
        {
            ...
            if (me->IsWithinDistInMap(me->GetVictim(), (float)e.event.minMaxRepeat.min, (float)e.event.minMaxRepeat.max))
                ProcessTimedAction(e, e.event.minMaxRepeat.repeatMin, e.event.minMaxRepeat.repeatMax, me->GetVictim());
            break;
        }
        ...
        {
            TC_LOG_DEBUG("FIXME","SmartScript: Gossip Select:  menu %u action %u", var0, var1);//little help for scripters
        ...
        default:
            TC_LOG_ERROR("FIXME","SmartScript::ProcessEvent: Unhandled Event type %u", e.GetEventType());
            break;
    }
}
...
void SmartScript::FillScript(SmartAIEventList e, WorldObject* obj, AreaTriggerEntry const* at)
{
    ...
    {
        if (obj)
            TC_LOG_DEBUG("FIXME","SmartScript: EventMap for Entry %u is empty but is using SmartScript.", obj->GetEntry());
        if (at)
            TC_LOG_DEBUG("FIXME","SmartScript: EventMap for AreaTrigger %u is empty but is using SmartScript.", at->id);
        return;
    }
    ...
    if (mEvents.empty() && obj)
        TC_LOG_ERROR("FIXME","SmartScript: Entry %u has events but no events added to list because of instance flags.", obj->GetEntry());
    if (mEvents.empty() && at)
        TC_LOG_ERROR("FIXME","SmartScript: AreaTrigger %u has events but no events added to list because of instance flags. NOTE: triggers can not handle any instance flags.", at->id);
}
    ...
    if (obj)//handle object based scripts
    {
        switch (obj->GetTypeId())
        {
            case TYPEID_UNIT:
                mScriptType = SMART_SCRIPT_TYPE_CREATURE;
                me = obj->ToCreature();
                TC_LOG_DEBUG("FIXME","SmartScript::OnInitialize: source is Creature %u", me->GetEntry());
                break;
            case TYPEID_GAMEOBJECT:
                mScriptType = SMART_SCRIPT_TYPE_GAMEOBJECT;
                go = obj->ToGameObject();
                TC_LOG_DEBUG("FIXME","SmartScript::OnInitialize: source is GameObject %u", go->GetEntry());
                break;
            default:
                TC_LOG_ERROR("FIXME","SmartScript::OnInitialize: Unhandled TypeID !WARNING!");
                return;
        }
    } else if (at)
    {
        ...
        trigger = at;
        TC_LOG_DEBUG("FIXME","SmartScript::OnInitialize: source is AreaTrigger %u", trigger->id);
    }
    else
    {
        TC_LOG_ERROR("FIXME","SmartScript::OnInitialize: !WARNING! Initialized objects are NULL.");
        return;
    }
...
void SmartScript::SetScript9(SmartScriptHolder& e, uint32 entry)
{
    ...
    {
        TC_LOG_ERROR("FIXME","SAI : Entry %d SourceType %u Event %u Action %u is trying to overwrite timed action list from a timed action, this is not allowed!.", e.entryOrGuid, e.GetScriptType(), e.GetEventType(), e.GetActionType());
        return;
    }
    mTimedActionList.clear();
