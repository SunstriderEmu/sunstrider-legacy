...
class TrinityStringTextBuilder 
{
    public:
        ...
        {
            std::string text = objmgr.GetTrinityString(_textId, locale);
    ...
    switch (e.GetActionType())
    {
        ...
        case SMART_ACTION_ADD_QUEST:
        {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsPlayer(*itr))
                {
                    if (Quest const* q = objmgr.GetQuestTemplate(e.action.quest.quest))
                    {
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                ...
                if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*itr)->ToUnit()->HasAura(e.action.cast.spell))
                {
                    ...
                    if (e.action.cast.flags & SMARTCAST_COMBAT_MOVE)
                    {
                        ...
                        SpellEntry const* spellInfo = spellmgr.LookupSpell(e.action.cast.spell);
                    ...
                    sLog.outDebug("Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (Guid: " UI64FMTD " Entry: %u Type: %u) already has the aura", e.action.cast.spell, (*itr)->GetGUID(), (*itr)->GetEntry(), uint32((*itr)->GetTypeId()));
            }
        ...
        */
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (Creature* npc = (*itr)->ToCreature())
                {
                   ...
                   /* int8 equipId = (int8)e.action.equip.entry;
                    if (equipId)
                    {
                        EquipmentInfo const* einfo = objmgr.GetEquipmentInfo(npc->GetEntry(), equipId);
                        if (!einfo)
                        {
                            sLog.outErrorDb("SmartScript: SMART_ACTION_EQUIP uses non-existent equipment info id %u for creature %u", equipId, npc->GetEntry());
                            break;
                        }
            ...
            for (ObjectList::const_iterator itr = casters->begin(); itr != casters->end(); ++itr)
            {
                ...
                for (ObjectList::const_iterator it = targets->begin(); it != targets->end(); ++it)
                {
                    ...
                    if (!(e.action.cast.flags & SMARTCAST_AURA_NOT_PRESENT) || !(*it)->ToUnit()->HasAura(e.action.cast.spell))
                    {
                        ...
                        sLog.outDebug("Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (Guid: " UI64FMTD " Entry: %u Type: %u) already has the aura", e.action.cast.spell, (*it)->GetGUID(), (*it)->GetEntry(), uint32((*it)->GetTypeId()));
                }
            }
            ...
            for (ObjectList::const_iterator itr = targets->begin(); itr != targets->end(); ++itr)
            {
                if (IsCreature(*itr))
                {
                             ...
                             e.GetTargetType() == SMART_TARGET_CLOSEST_ENEMY || e.GetTargetType() == SMART_TARGET_CLOSEST_FRIENDLY)
                    {
                        ...
                        sLog.outErrorDb("SmartScript: Action target for SMART_ACTION_SET_HOME_POS is invalid, skipping");
                }
            }
        ...
        case SMART_ACTION_GAME_EVENT_STOP:
        {
            ...
            {
                sLog.outErrorDb("SmartScript::ProcessAction: At case SMART_ACTION_GAME_EVENT_STOP, inactive event (id: %u)", eventId);
                break;
            }
        ...
        case SMART_ACTION_GAME_EVENT_START:
        {
            ...
            if (gameeventmgr.IsActiveEvent(eventId))
            {
                sLog.outErrorDb("SmartScript::ProcessAction: At case SMART_ACTION_GAME_EVENT_START, already activated event (id: %u)", eventId);
                break;
            }
