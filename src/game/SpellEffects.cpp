    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_WARRIOR:
            ...
            if(m_spellInfo->SpellFamilyFlags & 0x20000000)
            {
                ...
                spell_id = 20647;
                bp = damage+int32(m_caster->GetPower(POWER_RAGE) * m_spellInfo->Effects[i].DamageMultiplier);
...
void Spell::EffectTriggerSpellWithValue(uint32 i)
{
    ...
    if(!spellInfo)
    {
        TC_LOG_ERROR("FIXME","EffectTriggerSpellWithValue of spell %u: triggering unknown spell id %i\n", m_spellInfo->Id,triggered_spell_id);
        return;
    }
...
void Spell::SpellDamageHeal(uint32 /*i*/)
{
    ...
    if( unitTarget && unitTarget->IsAlive() && damage >= 0)
    {
        ...
        if (m_spellInfo->Id == 20167)
        {
            ...
            int32 holy = caster->SpellBaseHealingBonus(m_spellInfo->GetSchoolMask());
...
void Spell::EffectDispel(uint32 i)
{
    ...
    if (!dispel_list.empty())
    {
        ...
        if (!success_list.empty())
        {
            ...
            // On succes dispel
            // Devour Magic
            if (m_spellInfo->SpellFamilyName == SPELLFAMILY_WARLOCK && m_spellInfo->Category->Id == SPELLCATEGORY_DEVOUR_MAGIC)
            {
                uint32 heal_spell = 0;
                switch (m_spellInfo->Id)
                {
...
void Spell::SpellDamageWeaponDmg(uint32 i)
{
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            {
                spell_bonus += int32(0.20f*m_caster->SpellBaseDamageBonus(m_spellInfo->GetSchoolMask()));
                spell_bonus += int32(0.29f*m_caster->SpellBaseDamageBonusForVictim(m_spellInfo->GetSchoolMask(), unitTarget));
            }
            break;
        }
        case SPELLFAMILY_SHAMAN:
        ...
        if (unitTarget->m_currentSpells[i])
        {
            ...
            if ( (unitTarget->m_currentSpells[i]->getState() == SPELL_STATE_CASTING || (unitTarget->m_currentSpells[i]->getState() == SPELL_STATE_PREPARING && unitTarget->m_currentSpells[i]->GetCastTime() > 0.0f)) && unitTarget->m_currentSpells[i]->m_spellInfo->InterruptFlags & SPELL_INTERRUPT_FLAG_INTERRUPT && unitTarget->m_currentSpells[i]->m_spellInfo->PreventionType == SPELL_PREVENTION_TYPE_SILENCE )
            {
                if(m_originalCaster)
                {
                    ...
                    unitTarget->ProhibitSpellSchool(unitTarget->m_currentSpells[i]->m_spellInfo->GetSchoolMask(), duration/*GetSpellDuration(m_spellInfo)*/);
                }
...
void Spell::EffectSummonObject(uint32 i)
{
    ...
    Map *map = m_caster->GetMap();
    if(!pGameObj->Create(sObjectMgr->GenerateLowGuid(HIGHGUID_GAMEOBJECT,true), go_id, map, x, y, z, m_caster->GetOrientation(), 0, 0, rot2, rot3, 0, 1))
    {
        delete pGameObj;
        return;
    }
...
void Spell::EffectSkinning(uint32 /*i*/)
{
    ...
    creature->RemoveUnitMovementFlag(MOVEMENTFLAG_CAN_FLY | MOVEMENTFLAG_FLYING);
...
void Spell::EffectSummonCritter(uint32 i)
{
    ...
    {
        TC_LOG_ERROR("FIXME","Spell::EffectSummonCritter, spellid %u: no such creature entry %u", m_spellInfo->Id, pet_entry);
        delete critter;
        return;
    }
    ...
    {
        TC_LOG_ERROR("FIXME","ERROR: Pet (guidlow %d, entry %d) not summoned. Suggested coordinates isn't valid (X: %f Y: %f)",
...
void Spell::EffectTransmitted(uint32 effIndex)
{
    ...
    if (!goinfo)
    {
        TC_LOG_ERROR("FIXME","Gameobject (Entry: %u) not exist and not created at spell (ID: %u) cast",name_id, m_spellInfo->Id);
        return;
    }
