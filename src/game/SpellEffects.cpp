...
void Spell::EffectDispel(uint32 i)
{
    ...
    if (!dispel_list.empty())
    {
        ...
        for (int32 count=0; count < damage && list_size > 0; ++count)
        {
            ...
            SpellEntry const* spellInfo = aur->GetSpellInfo();
            // Base dispel chance
    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_WARRIOR:
        {
            ...
            if(m_spellInfo->SpellVisual == 671 && m_spellInfo->SpellIconID == 1508)
            {
                ...
                {
                    SpellEntry const *proto = (*itr)->GetSpellInfo();
    ...
    if( m_spellInfo->SpellFamilyName == SPELLFAMILY_PALADIN )
    {
        switch(m_spellInfo->SpellFamilyFlags)
        {
            ...
            case 0x800000:
            {
                ...
                {
                    SpellEntry const *spellInfo = (*itr)->GetSpellInfo();
...
void Spell::EffectDispelMechanic(uint32 i)
{
    ...
    for(Unit::AuraMap::iterator iter = Auras.begin(), next; iter != Auras.end(); iter = next)
    {
        next = iter;
        ++next;
        SpellEntry const *spell = sSpellMgr->GetSpellInfo(iter->second->GetSpellInfo()->Id);
