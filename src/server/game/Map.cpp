...
void Map::LoadMap(uint32 mapid, uint32 instanceid, int x,int y)
{
    if( instanceid != 0)
    {
        ...
        if (!m_parentMap->GridMaps[x][y])
			m_parentMap->EnsureGridCreated(GridPair(63-x,63-y));

//+++        if (!baseMap->GridMaps[x][y])  don't check for GridMaps[gx][gy], we need the management for vmaps
//            return;

        ((MapInstanced*)(m_parentMap))->AddGridMapReference(GridPair(x,y));
...
{
    // add to world object registry in grid
    if(obj->IsPet() || obj->IsWorldObject())
...
void
Map::EnsureGridLoaded(const Cell& cell)
{
...
void Map::AddCreatureToMoveList(Creature *c, float x, float y, float z, float ang)
{
    ...
    _creaturesToMove[c] = ObjectMover(x,y,z,ang);
}
...
void Map::AddGameObjectToMoveList(GameObject* go, float x, float y, float z, float ang)
{
    ...
    _gameObjectsToMove[go] = ObjectMover(x,y,z,ang);
}

void Map::MoveAllCreaturesInMoveList()
{
	for (auto iter : _creaturesToMove)
    {
...
#endif
            //CreatureRelocationNotify(c,new_cell,new_cell.GetCellCoord());
            AddUnitToNotify(c);
        }
        else
        {
...
{
    for(auto iter : _gameObjectsToMove)
    {
