...
inline float GetAge(uint64 t) { return float(WorldGameTime::GetGameTime() - t) / DAY; }

///////////////////////////////////////////////////////////////////////////////////////////////////
// GM ticket
GmTicket::GmTicket() : _id(0), _type(TICKET_TYPE_OPEN), _posX(0), _posY(0), _posZ(0), _mapId(0), _createTime(0), _lastModifiedTime(0),
                       _closedTime(0), _completed(false), _escalatedStatus(TICKET_UNASSIGNED), _viewed(false),
                       _needResponse(false), _needMoreHelp(false) { }

GmTicket::GmTicket(Player* player) : _type(TICKET_TYPE_OPEN), _posX(0), _posY(0), _posZ(0), _mapId(0), _createTime(WorldGameTime::GetGameTime()), _lastModifiedTime(WorldGameTime::GetGameTime()),
                       _closedTime(0), _completed(false), _escalatedStatus(TICKET_UNASSIGNED), _viewed(false),
                       _needResponse(false), _needMoreHelp(false)
{
...
bool GmTicket::LoadFromDB(Field* fields)
{
    ...
    _closedTime         = fields[++index].GetUInt32();
...
void GmTicket::SaveToDB(SQLTransaction& trans) const
{
    ...
    // id, type, playerGuid, name, description, createTime, mapId, posX, posY, posZ, lastModifiedTime, closedBy, closedTime, assignedTo, comment, response, completed, escalated, viewed, needMoreHelp, resolvedBy
    uint8 index = 0;
    ...
    stmt->setUInt32(++index, uint32(_closedTime));
...
{
    _message = message;
    _lastModifiedTime = uint64(WorldGameTime::GetGameTime());
}
...
{ 
    ...
    _closedTime = uint64(WorldGameTime::GetGameTime());
}
...
TicketMgr::TicketMgr() : _status(true), _lastTicketId(0), _lastSurveyId(0), _openTicketCount(0),
    _lastChange(WorldGameTime::GetGameTime()) { }
...
{
    _lastChange = uint64(WorldGameTime::GetGameTime());
}
