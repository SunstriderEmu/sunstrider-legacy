...
{
    uint8 count = 128;
    WorldPacket data(SMSG_INITIALIZE_FACTIONS, 4 + count * 5); //LK & BC ok
    data << uint32(count);
...
void ReputationMgr::DropFactionReputation(uint32 faction_id)
{
    FactionStateList::iterator itr = _factions.find(faction_id);
    if (itr != _factions.end())
    {
        itr->second.needDelete = true;
        itr->second.Standing = 0;
    }
}

void ReputationMgr::SwapFactionReputation(uint32 from_faction_id, uint32 to_faction_id)
{
    FactionEntry const* fromFactionEntry = sFactionStore.LookupEntry(from_faction_id);
    FactionEntry const* toFactionEntry = sFactionStore.LookupEntry(to_faction_id);
    if (!fromFactionEntry || !toFactionEntry)
    {
        TC_LOG_ERROR("entities.player", "ReputationMgr::SwapFactionReputation: Attempt to swap a faction with a non-existing FactionEntry");
        return;
    }

    uint32 fromRep = GetReputation(fromFactionEntry);
    uint32 toRep = GetReputation(toFactionEntry);
    SetReputation(fromFactionEntry, toRep);
    SetReputation(toFactionEntry, fromRep);
}
...
void ReputationMgr::SaveToDB(SQLTransaction& trans)
{
    for (FactionStateList::iterator itr = _factions.begin(); itr != _factions.end(); ++itr)
    {
        if (itr->second.needDelete)
        {
            ...
            itr->second.needDelete = false;
        }
        else if (itr->second.needSave)
        {
            PreparedStatement* stmt = CharacterDatabase.GetPreparedStatement(CHAR_DEL_CHAR_REPUTATION_BY_FACTION);
            stmt->setUInt32(0, _player->GetGUID().GetCounter());
            stmt->setUInt16(1, uint16(itr->second.ID));
