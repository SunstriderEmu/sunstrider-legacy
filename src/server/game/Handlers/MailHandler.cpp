...
bool WorldSession::CanOpenMailBox(ObjectGuid guid)
{
    ...
    {
        if (/*TC!HasPermission(rbac::RBAC_PERM_COMMAND_MAILBOX)*/ !_player->IsGameMaster())
        {
...
void WorldSession::HandleSendMail(WorldPacket& recvData) //LK ok
{
...
/*TC
    ...
    */
    ...
    if (!accountBound && player->GetTeam() != receiverTeam && /*TC!HasPermission(rbac::RBAC_PERM_TWO_SIDE_INTERACTION_MAIL)*/ !player->IsGameMaster())
    {
...
//called when mail is read / LK ok
...
//called when client deletes mail / LK ok
...
void WorldSession::HandleMailReturnToSender(WorldPacket& recvData) //LK ok
{
    ObjectGuid mailbox;
    uint32 mailId;
    recvData >> mailbox;
    recvData >> mailId;
    recvData.read_skip<uint64>();                          // original sender GUID for return to, not used (is this in BC packet too?)
    ...
    Player* player = _player;
    Mail* m = player->GetMail(mailId);
    if (!m || m->state == MAIL_STATE_DELETED || m->deliver_time > WorldGameTime::GetGameTime())
    {
        player->SendMailResult(mailId, MAIL_RETURNED_TO_SENDER, MAIL_ERR_INTERNAL_ERROR);
        return;
    }

    std::string body = m->itemTextId ? sObjectMgr->GetItemText(m->itemTextId) : "";
    ...
    {
        MailDraft draft(m->subject, body);
        if (m->mailTemplateId)
...
//called when player takes item attached in mail (LK ok)
...
void WorldSession::HandleMailTakeMoney(WorldPacket& recvData) //LK ok
{
    ObjectGuid mailbox;
    uint32 mailId;
    recvData >> mailbox;
...
void WorldSession::HandleGetMailList(WorldPacket& recvData) //LK ok
{
    ObjectGuid mailbox;
    recvData >> mailbox;
    ...
    time_t cur_time = WorldGameTime::GetGameTime();
...
#ifndef LICH_KING
        data << uint32((*itr)->itemTextId);
#endif
        data << uint32(0);                                   // package (Package.dbc)
        data << uint32((*itr)->stationery);                  // stationery (Stationery.dbc)
        data << uint32((*itr)->money);                       // Gold
        data << uint32((*itr)->checked);                     // flags
        data << float(float((*itr)->expire_time - WorldGameTime::GetGameTime())/DAY); // Time
...
/** Corecraft comment:
* Handles the packet sent by the client when he copies the body a mail to his
* inventory.
*
* When a player copies the body of a mail to his inventory this method is
* called.It will create a new item with the text of the mail and store it in the players inventory
* (if possible).
**/
void WorldSession::HandleMailCreateTextItem(WorldPacket& recvData)
{
    ...
    recvData >> mailbox;
    recvData >> mailId;
    recvData.read_skip<uint32>(); // mailTemplateId, non need, Mail store own
                                   // 100% correct value anyway
    ...
    std::string body;
    if(m)
        body = m->itemTextId ? sObjectMgr->GetItemText(m->itemTextId) : "";
    if (!m || (body.empty() && !m->mailTemplateId) || m->state == MAIL_STATE_DELETED || m->deliver_time > WorldGameTime::GetGameTime() || (m->checked & MAIL_CHECK_MASK_COPIED))
    {
    ...
    ItemTemplate const* itemProto = sObjectMgr->GetItemTemplate(MAIL_BODY_ITEM_TEMPLATE);
    if (!itemProto)
    {
        player->SendMailResult(mailId, MAIL_MADE_PERMANENT, MAIL_ERR_INTERNAL_ERROR);
        return;
    ...
    if (!bodyItem->Create(sObjectMgr->GetGenerator<HighGuid::Item>().Generate(), MAIL_BODY_ITEM_TEMPLATE, player, itemProto))
    {
        delete bodyItem;
        player->SendMailResult(mailId, MAIL_MADE_PERMANENT, MAIL_ERR_INTERNAL_ERROR);
        return;
    ...
    if (m->mailTemplateId)
    {
        ...
        bodyItem->SetUInt32Value(ITEM_FIELD_ITEM_TEXT_ID, sObjectMgr->CreateItemText(mailTemplateEntry->content[GetSessionDbcLocale()]));
    }
    else
        bodyItem->SetUInt32Value(ITEM_FIELD_ITEM_TEXT_ID, body.empty() ? 0 : sObjectMgr->CreateItemText(body));
...
void WorldSession::HandleQueryNextMailTime(WorldPacket & /*recvData*/) //LK ok
{
