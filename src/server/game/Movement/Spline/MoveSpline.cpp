...
#define CHECK(exp) \
    if (!(exp))\
    {\
        TC_LOG_ERROR("misc", "MoveSplineInitArgs::Validate: expression '%s' failed for GUID: %u Entry: %u", #exp, unit->GetTypeId() == TYPEID_PLAYER ? unit->GetGUIDLow() : unit->ToCreature()->GetSpawnId(), unit->GetEntry());\
        return false;\
    }
    CHECK(path.size() > 1);
