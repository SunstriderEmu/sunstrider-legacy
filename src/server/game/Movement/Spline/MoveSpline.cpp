...
#define CHECK(exp) \
    ...
    {\
        if (unit)\
           TC_LOG_ERROR("misc", "MoveSplineInitArgs::Validate: expression '%s' failed for SpawnId: %u Entry: %u", #exp, unit->GetTypeId() == TYPEID_PLAYER ? unit->GetGUID().GetCounter() : unit->ToCreature()->GetSpawnId(), unit->GetEntry());\
       else\
           TC_LOG_ERROR("misc", "MoveSplineInitArgs::Validate: expression '%s' failed for cyclic spline continuation", #exp);\
        return false;\
    }
    CHECK(path.size() > 1);
