...
void SplineBase::init_spline(const Vector3 * controls, index_type count, EvaluationMode m, float initialOrientation)
{
    m_mode = m;
    cyclic = false;

    (this->*initializers[m_mode])(controls, count, cyclic, 0, initialOrientation);
}

void SplineBase::init_cyclic_spline(const Vector3 * controls, index_type count, EvaluationMode m, index_type cyclic_point, float initialOrientation)
{
    m_mode = m;
    cyclic = true;

    (this->*initializers[m_mode])(controls, count, cyclic, cyclic_point, initialOrientation);
}

void SplineBase::InitLinear(const Vector3* controls, index_type count, bool localCyclic, index_type cyclic_point, float /*initialOrientation*/)
{
    ASSERT(count >= 2);
...
void SplineBase::InitCatmullRom(const Vector3* controls, index_type count, bool localCyclic, index_type cyclic_point, float initialOrientation)
{
...
void SplineBase::InitBezier3(const Vector3* controls, index_type count, bool /*cyclic*/, index_type /*cyclic_point*/, float /*initialOrientation*/)
{
