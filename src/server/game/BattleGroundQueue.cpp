...
*/
void BattlegroundQueue::BattlegroundQueueUpdate(BattlegroundTypeId bgTypeId, uint32 queue_id, uint8 arenatype, bool isRated, uint32 arenaRating)
{
    ...
    for (BGFreeSlotQueueContainer::iterator itr = bgQueues.begin(); itr != bgQueues.end();)
    {
        ...
        ++itr; //already increment here because ?
        // battleground is running, so if:
        // DO NOT allow queue manager to invite new player to running arena
        if (bg->isBattleground() && bg->GetTypeID() == bgTypeId && bg->GetQueueType() == queue_id 
            && bg->GetStatus() > STATUS_WAIT_QUEUE && bg->GetStatus() < STATUS_WAIT_LEAVE)
        {
            //we must check both teams
            for(auto& queuedGroup : m_QueuedGroups[queue_id])
            {
                // did the group join for this bg type?
                if(queuedGroup->BgTypeId != bgTypeId)
                    continue;

                // if so, check if fits in
                if(bg->GetFreeSlotsForTeam(queuedGroup->Team) >= queuedGroup->Players.size())
                {
                    // if group fits in, invite it
                    InviteGroupToBG(queuedGroup, bg, queuedGroup->Team);
                }
                // invite group if it has enough free slots in absolute (not with GetFreeSlotsForTeam)
                else if (queuedGroup->Players.size() > 1 && (bg->GetInvitedCount(queuedGroup->Team) + queuedGroup->Players.size()) < bg->GetMaxPlayersPerTeam()) {
                    InviteGroupToBG(queuedGroup, bg, queuedGroup->Team);
                }
            }
