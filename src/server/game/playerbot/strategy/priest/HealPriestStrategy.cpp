...
void HealPriestStrategy::InitTriggers(std::list<TriggerNode*> &triggers)
{
    ...
    triggers.push_back(new TriggerNode(
        "almost full health",
        NextAction::array({ new NextAction("renew", 15.0f) })));

    triggers.push_back(new TriggerNode(
        "party member almost full health",
        NextAction::array({ new NextAction("renew on party", 10.0f) })));

    triggers.push_back(new TriggerNode(
        "enemy too close for spell",
        NextAction::array({ new NextAction("fade", 50.0f), new NextAction("flee", 49.0f) })));
}
