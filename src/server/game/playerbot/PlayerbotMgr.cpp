...
void PlayerbotHolder::OnBotLogin(Player * const bot)
{
    ...
    if (master)
    {
        ObjectGuid masterGuid = ObjectGuid(master->GetGUID());
        if (master->GetGroup() &&
...
list<std::string> PlayerbotHolder::HandlePlayerbotCommand(char const* args, Player* master)
{
    ...
    if (charnameStr == "!" && master && master->GetSession()->GetSecurity() > SEC_GAMEMASTER1)
    {
    ...
    for (set<std::string>::iterator i = bots.begin(); i != bots.end(); ++i)
    {
        ...
        ObjectGuid member = ObjectGuid(sWorld->GetCharacterGuidByName(bot));
        if (!member)
        {
