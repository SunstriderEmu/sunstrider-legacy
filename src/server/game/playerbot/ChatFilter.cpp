...
public:
    ...
    virtual std::string Filter(std::string message)
    {
        ...
        if (level > 0 && bot->GetLevel() == uint32(level))
            return ChatFilter::Filter(message);
...
public:
    ...
    virtual std::string Filter(std::string message)
    {
        ...
        bool found = false;
        for (list<std::string>::iterator i = rtis.begin(); i != rtis.end(); i++)
        {
            std::string rti = *i;
            ...
            ObjectGuid rtiTarget = ObjectGuid(group->GetTargetIcon(RtiTargetValue::GetRtiIndex(rti.substr(1))));
