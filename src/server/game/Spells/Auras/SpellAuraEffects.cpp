...
void AuraEffect::HandlePeriodicHealthLeechAuraTick(Unit* m_target, Unit* caster, uint32 &realDamage) const
{
    ...
    pdamage = caster->SpellDamageBonusDone(m_target, GetSpellInfo(), pdamage, DOT);
    ...
    uint32 heal = caster->SpellHealingBonusDone(caster, spellProto, uint32(new_damage * multiplier), DOT);
            ...
            if (!apply && (mode & AURA_EFFECT_HANDLE_REAL))
            {
                if (GetBase()->GetDuration() <= 0 || aurApp->GetRemoveMode() == AURA_REMOVE_BY_ENEMY_SPELL)
                {
                    ...
                    if (m_target->IsInWorld())
                    {
                        ...
                        if(caster)
                            amount = caster->SpellHealingBonusDone(m_target, GetSpellInfo(), _amount, HEAL);
...
void AuraEffect::HandleProcTriggerSpellAuraProc(AuraApplication* aurApp, ProcEventInfo& eventInfo)
{
    ...
    //hack fest (when removing this, remove OldWindrunnerHacks in parallel)
    {
        ...
        case SPELL_AURA_MANA_SHIELD: 
        {
            ...
            {
                // Magic Absorption
                if (GetSpellInfo()->SpellIconID == 459)             // only this spell have SpellIconID == 459 and dummy aura
                {
                    // mana reward
                    basepoints0 = (triggeredByAura->GetAmount() * triggerCaster->GetMaxPower(POWER_MANA) / 100);
                    triggerTarget = triggerCaster;
                    triggered_spell_id = 29442;
                    break;
                }
                // Master of Elements
                if (GetSpellInfo()->SpellIconID == 1920)
                {
                    // mana cost save
                    basepoints0 = procSpell->ManaCost * triggeredByAura->GetAmount() / 100;
                    if (basepoints0 <= 0)
                        return;
                ...
                if (GetSpellInfo()->SpellFamilyFlags & 0x0000000000008000LL)
                {
                    triggerTarget = triggerCaster;
                ...
                case 12848:
                {
                    ...
                    default:
                        TC_LOG_ERROR("spell", "Unit::HandleDummyAuraProc: non handled spell id: %u (IG)", GetSpellInfo()->Id);
                        return;
                    }

                    auto const &DoT = triggerTarget->GetAuraEffectsByType(SPELL_AURA_PERIODIC_DAMAGE);

                    for (auto itr : DoT)
                        if (itr->GetId() == 12654 && itr->GetCaster() == triggerCaster)
                        {
                            uint32 m_currentBasePoints = m_spellInfo->Effects[itr->GetEffIndex()].BasePoints;
                            if (m_currentBasePoints > 0)
                                basepoints0 += int(m_currentBasePoints / (itr->GetTickNumber() + 1));
                        }
                ...
                case 11129:
                {
                    ...
                    if (triggeredByAura->GetBase()->m_procCharges <= 1 && (procEx & PROC_HIT_CRITICAL))
                    {
                        triggerCaster->RemoveAurasDueToSpell(28682);       //-> remove Combustion auras
                        return;                        // charge counting (will removed)
                    }
                    ...
                    triggerCaster->CastSpell(triggerCaster, 28682, args);
