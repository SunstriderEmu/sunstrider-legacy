...
namespace Trainer
{
    ...
    {
    public:
        Trainer(uint32 trainerId, Type type, uint32 requirement, std::string greeting, std::vector<TrainerSpell> spells);
        ...
        std::vector<TrainerSpell> const& GetSpells() const { return _spells; }

        static SpellState GetSpellState(Player const* player, TrainerSpell const* trainerSpell);
    private:
        TrainerSpell const* GetSpell(uint32 spellId) const;
        bool CanTeachSpell(Player const* player, TrainerSpell const* trainerSpell) const;
        ...
        Type _type;
        uint32 _requirement;
        std::vector<TrainerSpell> _spells;
