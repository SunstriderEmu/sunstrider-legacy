...
bool GossipSelect_npc_fallen_hero_of_horde(Player* pPlayer, Creature* pCreature, uint32 sender, uint32 action)
{
    switch (action)
    {
        case GOSSIP_ACTION_INFO_DEF+1:
            ...
            pPlayer->SEND_GOSSIP_MENU_TEXTID(1392, pCreature->GetGUID());
            break;
...
void AddSC_blasted_lands()
{
    ...
    newscript->OnGossipHello =  &GossipHello_npc_deathly_usher;
    newscript->OnGossipSelect = &GossipSelect_npc_deathly_usher;
    sScriptMgr->RegisterOLDScript(newscript);

    newscript = new OLDScript;
    newscript->Name="npc_fallen_hero_of_horde";
    newscript->OnGossipHello =  &GossipHello_npc_fallen_hero_of_horde;
    newscript->OnGossipSelect = &GossipSelect_npc_fallen_hero_of_horde;
    sScriptMgr->RegisterOLDScript(newscript);
}
