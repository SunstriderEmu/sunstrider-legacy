...
bool GossipHello_boss_gloomrel(Player *player, Creature *_Creature)
{
    ...
    player->SEND_GOSSIP_MENU_TEXTID(2601, _Creature->GetGUID());
...
bool GossipSelect_boss_gloomrel(Player *player, Creature *_Creature, uint32 sender, uint32 action )
{
    switch (action)
    {
        case GOSSIP_ACTION_INFO_DEF+1:
            ...
            player->SEND_GOSSIP_MENU_TEXTID(2606, _Creature->GetGUID());
            break;
        ...
        case GOSSIP_ACTION_INFO_DEF+2:
            ...
            player->SEND_GOSSIP_MENU_TEXTID(2604, _Creature->GetGUID());
            break;
...
bool GossipSelect_boss_doomrel(Player *player, Creature *_Creature, uint32 sender, uint32 action )
{
    switch (action)
    {
        case GOSSIP_ACTION_INFO_DEF+1:
            ...
            player->SEND_GOSSIP_MENU_TEXTID(2605, _Creature->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF+2:
            player->CLOSE_GOSSIP_MENU();
            //start event here
            _Creature->SetFaction(FACTION_HOSTILE);
            _Creature->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_IMMUNE_TO_PC);
...
void AddSC_boss_tomb_of_seven()
{
    ...
    newscript->OnGossipHello = &GossipHello_boss_gloomrel;
    newscript->OnGossipSelect = &GossipSelect_boss_gloomrel;
    sScriptMgr->RegisterOLDScript(newscript);

    newscript = new OLDScript;
    newscript->Name = "boss_doomrel";
    newscript->GetAI = &GetAI_boss_doomrel;
    newscript->OnGossipHello = &GossipHello_boss_doomrel;
    newscript->OnGossipSelect = &GossipSelect_boss_doomrel;
    sScriptMgr->RegisterOLDScript(newscript);
}
