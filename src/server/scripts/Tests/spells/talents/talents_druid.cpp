...
// We calculated the Cat min damage based on videos and informations from WoWWiki/WoWhead
uint32 GetCatMinDmg(uint32 level = 70)
{
    return std::min(level, 60u) * 0.8f - 4;
}

uint32 GetCatMaxDmg(uint32 level = 70)
{
    return GetCatMinDmg(level) * 1.5f;
}
...
public:
	...
	public:
		...
		void Test() override
		{
            ...
            TEST_CAST(druid, druid, ClassSpells::Druid::NATURES_GRASP_RNK_7);
			ASSERT_INFO("Druid has not Nature's Grasp aura");
            TEST_HAS_AURA(druid, ClassSpells::Druid::NATURES_GRASP_RNK_7);
            warrior->ForceMeleeHitResult(MELEE_HIT_NORMAL);
			warrior->AttackerStateUpdate(druid, BASE_ATTACK);
			ASSERT_INFO("Druid still has aura");
            TEST_HAS_NOT_AURA(druid, ClassSpells::Druid::NATURES_GRASP_RNK_7);
...
public:
	...
	public:
		...
		void Test() override
		{
            ...
            TEST_MELEE_HIT_CHANCE_CALLBACK(rogue, druid, BASE_ATTACK, expectedMissChance, MELEE_HIT_MISS, [](Unit* attacker, Unit* victim) {
...
public:
	...
	public:
		...
		void Test() override
		{
			...
			float const minDamage = GetCatMinDmg() + AP / 14;
			float const maxDamage = GetCatMaxDmg() + AP / 14;
			float const savageFuryFactor = 1.2f;

			// Mangle (Cat)
            uint32 const mangleDmgBonus = 264;
            uint32 const mangleDmgCoeff = 1.6f;
...
public:
	...
	public:
		...
		void TestMangle(TestPlayer* druidWithMangle, TestPlayer* druidTestDamage, Unit* creature, uint32 mangleSpellId)
		{
			...
			uint32 const minDamage = GetCatMinDmg() + AP / 14;
			uint32 const maxDamage = GetCatMaxDmg() + AP / 14;
