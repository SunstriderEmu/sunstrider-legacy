...
static bool HandleRecupReputations(Player *player, std::string reputs)
{
    ...
    for (i = vline.begin(); i != vline.end(); i++) {
        ...
        handler->PSendSysMessage(LANG_COMMAND_MODIFY_REP, factionEntry->name[GetSessionDbcLocale()], factionId, player->GetName().c_str(), player->GetReputation(factionId));
    }
