...
static bool HandleGoTicketCommand(const char * args)
{
    ARGS_CHECK
...
static bool HandleGoZoneXYCommand(ChatHandler* handler, char const* args)
{
    ...
    if(map->Instanceable())
    {
        handler->PSendSysMessage(LANG_INVALID_ZONE_MAP,areaEntry->ID,areaEntry->area_name[GetSessionDbcLocale()],map->GetId(),map->GetMapName());
        handler->SetSentErrorMessage(true);
        return false;
    }
