...
static bool HandleDebugInArcCommand(const char* /*args*/)
{
...
static bool HandleDebugSendOpcodeCommand(const char* /*args*/)
{
...
static bool HandleDebugArenaCommand(const char * /*args*/)
{
...
static bool HandleDebugThreatListCommand(const char * args)
{
    Unit* target = GetSelectedUnit();
    if (!target)
...
static bool HandleDebugCombatListCommand(const char * /*args*/)
{
    Unit* target = GetSelectedUnit();
    if (!target)
        target = GetSession()->GetPlayer();
...
{
    handler->PSendSysMessage("Total stuff level: %u", m_session->GetPlayer()->GetEquipedItemsLevelSum());
...
static bool HandleDebugGetLootRecipient(const char* /*args*/)
{
...
static bool HandleDebugAttackDistance(ChatHandler* handler, char const* args)
{
    ...
    handler->PSendSysMessage("AttackDistance: %f - ModDetectRange: %i", target->ToCreature()->GetAggroRange(m_session->GetPlayer()), target->GetTotalAuraModifier(SPELL_AURA_MOD_DETECT_RANGE));
    return true;
}
...
static bool HandleDebugLoSCommand(ChatHandler* handler, char const* args)
{
    if (Unit* unit = GetSelectedUnit())
        handler->PSendSysMessage("Unit %s (GuidLow: %u) is %sin LoS", unit->GetName().c_str(), unit->GetGUID().GetCounter(), m_session->GetPlayer()->IsWithinLOSInMap(unit) ? "" : "not ");
...
static bool HandleDebugOpcodeTestCommand(const char* /* args */)
{
...
static bool HandleDebugBoundaryCommand(char const* args)
{
