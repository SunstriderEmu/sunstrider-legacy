...
//kick player
static bool HandleKickPlayerCommand(const char *args)
{
...
static bool HandleDeMorphCommand(const char* /*args*/)
{
    Unit *target = GetSelectedUnit();
    if(!target)
        target = m_session->GetPlayer();
...
static bool HandleGUIDCommand(const char* /*args*/)
{
...
static bool HandleMaxSkillCommand(const char* /*args*/)
{
...
static bool HandleCooldownCommand(ChatHandler* handler, char const* args)
{
    ...
    else
    {
        ...
        {
            handler->PSendSysMessage(LANG_UNKNOWN_SPELL, target==m_session->GetPlayer() ? GetTrinityString(LANG_YOU) : target->GetName().c_str());
            handler->SetSentErrorMessage(true);
            return false;
        }

        target->GetSpellHistory()->ResetCooldown(spell_id, true);
        handler->PSendSysMessage(LANG_REMOVE_COOLDOWN, spell_id, target==m_session->GetPlayer() ? GetTrinityString(LANG_YOU) : target->GetName().c_str());
    }
    return true;
}
...
static bool HandleDieCommand(const char* /*args*/)
{
    Unit* target = GetSelectedUnit();
...
static bool HandleGetDistanceCommand(const char* /*args*/)
{
    ...
    handler->PSendSysMessage(LANG_DISTANCE, m_session->GetPlayer()->GetDistance(pUnit), m_session->GetPlayer()->GetDistance2d(pUnit));
    handler->PSendSysMessage("Exact distance 2d: %f", m_session->GetPlayer()->GetExactDistance2d(pUnit->GetPositionX(), pUnit->GetPositionY()));

    return true;
}

static bool HandleDamageCommand(const char * args)
{
    ARGS_CHECK
...
static bool HandleBankCommand(const char* /*args*/)
{
...
static bool HandleRespawnCommand(const char* /*args*/)
{
    Player* player = m_session->GetPlayer();
...
static bool HandleMovegensCommand(const char* /*args*/)
{
...
static bool HandlePLimitCommand(const char *args)
{
    if(*args)
    {
...
static bool HandleComeToMeCommand(const char *args)
{
...
static bool HandleSendMoneyCommand(ChatHandler* handler, char const* args)
{
    ...
    handler->PSendSysMessage(LANG_MAIL_SENT, receiverName.c_str());
    return true;
}
...
static bool HandleFlushArenaPointsCommand(const char * /*args*/)
{
...
static bool HandleFreezeCommand(const char *args)
{
    std::string name;
    Player* player;
...
static bool HandleUnFreezeCommand(const char *args)
{
    std::string name;
    Player* player;
    char* TargetName = strtok((char*)args, " "); //get entered name
...
static bool HandleTestsJoinCommand(const char* testName) 
{ 
...
static bool HandleYoloCommand(const char* /* args */)
{
