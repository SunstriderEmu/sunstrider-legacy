...
static bool HandleNpcGuidCommand(ChatHandler* handler, char const* args)
{
    ...
    handler->PSendSysMessage("GUID: %u", target->GetSpawnId());
...
static bool HandleNpcChangeEntryCommand(const char *args)
{
...
static bool HandleNpcInfoCommand(const char* /*args*/)
{
    Creature* target = GetSelectedCreature();
...
static bool HandleNpcFollowCommand(const char* /*args*/)
{
...
static bool HandleNpcUnFollowCommand(const char* /*args*/)
{
    Player *player = m_session->GetPlayer();
    Creature *creature = GetSelectedCreature();
