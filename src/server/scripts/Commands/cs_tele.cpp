...
static bool HandleTeleAddCommand(const char * args)
{
    ARGS_CHECK
...
static bool HandleTeleDelCommand(const char * args)
{
    ARGS_CHECK
...
static bool HandleTeleNameCommand(const char * args)
{
    ARGS_CHECK
    ...
    {
        handler->PSendSysMessage(LANG_TELEPORTING_TO, name.c_str(), GetTrinityString(LANG_OFFLINE), tele->name.c_str());
...
static bool HandleTeleGroupCommand(const char * args)
{
    ARGS_CHECK
...
static bool HandleGroupgoCommand(ChatHandler* handler, char const* args)
{
    ...
    for(GroupReference *itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next())
    {
        ...
        // before GM
        float x,y,z;
        m_session->GetPlayer()->GetClosePoint(x,y,z,pl->GetCombatReach());
...
static bool HandleTeleCommand(const char * args)
{
    ARGS_CHECK
