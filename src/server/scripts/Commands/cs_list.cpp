...
static bool HandleListAurasCommand (const char * /*args*/)
{
 ...
 //       ss_name << "|cffffffff|Hspell:" << aura->GetId() << "|h[" << name << "]|h|r"; //message does not appear, what's wrong? link format is the same on LK and BC
        ...
        handler->PSendSysMessage(LANG_COMMAND_TARGET_AURADETAIL, aura->GetId(), (GetSession() ? ss_name.str().c_str() : name),
...
static bool HandleListSpawnPointsCommand(const char * args)
{
...
static bool HandleListRespawnsCommand(const char * args)
{
    Player const* player = GetSession()->GetPlayer();
    Map const* map = player->GetMap();
    uint32 range = 0;
    ...
    if (range)
        ...
        handler->PSendSysMessage(/*LANG_LIST_RESPAWNS_ZONE*/"Listing %s respawns for %s (zone %u)", stringCreature, GetZoneName(zoneId, GetSessionDbcLocale()), zoneId);
    ...
    for (RespawnInfo* ri : respawns)
    {
        ...
        handler->PSendSysMessage("%u | %u | [%02u,%02u] | %s (%u) | %s", ri->spawnId, ri->entry, gridX, gridY, GetZoneName(ri->zoneId, GetSessionDbcLocale()), ri->zoneId, map->IsSpawnGroupActive(data->spawnGroupData->groupId) ? respawnTime.c_str() : "inactive");
    }
    ...
    if (range)
        ...
        handler->PSendSysMessage(/*LANG_LIST_RESPAWNS_ZONE*/"Listing %s respawns for %s (zone %u)", stringGameobject, GetZoneName(zoneId, GetSessionDbcLocale()), zoneId);
